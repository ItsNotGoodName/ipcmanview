...
CREATE TABLE `new_dahua_afero_files` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `file_id` integer NULL, `file_thumbnail_id` integer NULL, `email_attachment_id` integer NULL, `name` text NOT NULL, `created_at` datetime NOT NULL, `ready` boolean NOT NULL DEFAULT false, CONSTRAINT `0` FOREIGN KEY (`email_attachment_id`) REFERENCES `dahua_email_attachments` (`id`) ON UPDATE CASCADE ON DELETE SET NULL, CONSTRAINT `1` FOREIGN KEY (`file_thumbnail_id`) REFERENCES `dahua_file_thumbnails` (`id`) ON UPDATE CASCADE ON DELETE SET NULL, CONSTRAINT `2` FOREIGN KEY (`file_id`) REFERENCES `dahua_files` (`id`) ON UPDATE CASCADE ON DELETE SET NULL);
-- copy rows from old table "dahua_afero_files" to new temporary table "new_dahua_afero_files"
INSERT INTO `new_dahua_afero_files` (`id`, `file_id`, `file_thumbnail_id`, `email_attachment_id`, `name`, `created_at`, `ready`) SELECT `id`, `file_id`, `file_thumbnail_id`, `email_attachment_id`, `name`, `created_at`, IFNULL(`ready`, false) AS `ready` FROM `dahua_afero_files`;
