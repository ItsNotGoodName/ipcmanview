...
CREATE TABLE `new_dahua_streams` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `internal` boolean NOT NULL, `device_id` integer NOT NULL, `channel` integer NOT NULL, `subtype` integer NOT NULL, `name` text NOT NULL, `mediamtx_path` text NOT NULL, CONSTRAINT `0` FOREIGN KEY (`device_id`) REFERENCES `dahua_devices` (`id`) ON UPDATE CASCADE ON DELETE CASCADE);
-- copy rows from old table "dahua_streams" to new temporary table "new_dahua_streams"
INSERT INTO `new_dahua_streams` (`id`, `device_id`, `channel`, `subtype`, `name`, `mediamtx_path`, `internal`) SELECT `id`, `device_id`, `channel`, `subtype`, `name`, `mediamtx_path`, false FROM `dahua_streams`;
