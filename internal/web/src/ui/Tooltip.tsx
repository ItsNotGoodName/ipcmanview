...
export function TooltipContent(props: Omit<Tooltip.TooltipContentProps, "style">) {
  ...
  return <Tooltip.Portal>
    <Tooltip.Content
      class={cn(
        "bg-popover max-w-[var(--kb-popper-content-available-width)] origin-[var(--kb-tooltip-content-transform-origin)] text-popover-foreground animate-in fade-in-0 zoom-in-95 ui-not-expanded:animate-out ui-not-expanded:fade-out-0 ui-not-expanded:zoom-out-95 z-50 overflow-hidden rounded-md border px-3 py-1.5 text-sm shadow-md",
        props.class
      )}
      {...rest}
    />
